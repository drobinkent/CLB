...
class ONOSBmv2Switch(Switch):
    ...
    def __init__(self, name, json=None, debugger=True, loglevel="off", #loglevel="debug",
                 ...
                 gnmi=True, portcfg=True, onosdevid=None, stratum=False,
                 **kwargs):
    ...
    def bmv2Args(self):
        ...
        #args.append('--log-console')  #console and log file are mutually exclusive
        args.append('-L %s' % self.loglevel)
        args.append('--log-file /tmp/bmv2-%s.log --log-flush  ' % self.name)
        if self.pktdump:
        ...
        args.append('--grpc-server-addr 127.0.0.1:%s' % self.grpcPort)
